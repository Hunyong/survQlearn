...
gk.lm <- function(data, weight, formula, ...) {
  data$weight = weight
  lm(formula, data = data, weights = weight, ...)
}
# tmp.lm <- lm(event.time ~ action*(Z1 + Z2+ Z5), data = tmp[,,1] %>% as.data.frame, weights = rep(1,300))
gk.rf <- function(data, weight, formula, ...) {
  full <- complete.cases(get_all_vars(formula, data))
  rfsrc(formula, data = data[full, ], case.wt = weight[full], ...)
}

# formula.lm <- function(p, time.label = "event.time", action.label = "action") 
#   as.formula(paste(time.label, " ~ ", action.label, 
#                    " + lB + ", paste(paste0("Z", 1:p), collapse = " + ")))
# formula.rf <- function(p, time.label = "event.time", action.label = "action") 
#   as.formula(paste(time.label, " ~ ", action.label, 
#                    " + lB + ", paste(paste0("Z", 1:p), collapse = " + ")))
formula.lm <- formula.rf <- function(p) 
  as.formula(paste("Surv(T, delta) ~ A + lB + ", paste(paste0("Z", 1:p), collapse = " + ")))
...
gk <-
  function(common.formula, 
           ...
           # ntime = 100L,
           method = c("rf", "lm"),
           tau = if(!is.null(timepoints)) max(timepoints),
    ...
    if (!is.null(common.formula)) {
      ...
      stop("TBD") #!!!!!!!!
      # Check if Tx label list is properly provided and so on...
      #!!!!!!!!!!!!!!!!!!!!
    }
    ...
    #   times = apply(data[, Y.label], 1, sum, na.rm = TRUE)
    #   time.3q = quantile(times, 0.75)
    #   if (timeticks == "quadratic") {
    #     timepoints = seq(0, ceiling(sqrt(time.3q)), length.out = ntime)^2
    #   } else if (timeticks == "uniform") {
    #     timepoints = seq(0, ceiling(time.3q), length.out = ntime)
    #   } else if (timeticks == "exponential") {
    #     timepoints = exp(seq(0, ceiling(log(time.3q)), length.out = ntime))
    #   }
    # } else {
    ...
    # tmp1 <<- list(data = data, Y.label = Y.label, stage.label = stage.label,
    #               delta.label = delta.label, eligibility = eligibility,
    ...
    for (q in n.stages:1L) {
      ...
      #                                   do.trace = do.trace, ...)
      res$survRF[[q]] <- methodFun(data = data[eligibility[, q], ], 
                                   formula = formula.list.new[[q]], 
                                   weight = weight[eligibility[, q], q], ...)
      # storing the estimated optimal decision
      rule <- predict.opt(res$survRF[[q]], newdata = data[eligibility[, q], ],
    ...
    # overall value for the training set (only needed at the end of the iteration.)
    # if (value.crit %in% c("surv.prob", "surv.mean")) {
    #   St = mean(apply(rule$pred.value.St, 1, max))
    #   res$value.train = c(St = St)
    # }
    # if (value.crit %in% c("mean", "surv.mean")) {
    #   ET = mean(apply(rule$pred.value.mean, 1, max))
    #   res$value.train = c(res$value.train, ET = ET)
    # }
    # attr(res$value.train, "criterion") = list(criterion = value.crit, 
    #                                           value.s.time = if (critSurv) {value.s.time})
    # 
    cl <- match.call()
    cl[[1]] <- as.name("GK")
    res$call <- cl
...
predict.opt <- function(object, newdata, Tx.label, tie.method = c("random", "first", "NA")) {
  tie.method = match.arg(tie.method)
  ntest = dim(newdata)[1]
  ...
  predictor = switch(class(object)[1], 
                     "rfsrc" = function(...) predict(...)$predicted, 
                     "lm" = function(...) predict(...)$fitted,
                     "DTRreg" = function(...) exp(predict(...)))
  
  for (i in 1:n.Tx) {
    newdata2[, Tx.label] <- Tx.levels[i]
    output.value [ ,i] = predictor(object, newdata = newdata2)
  }
  optimal.Tx = output.value %>% apply(1, whichMax, tie.method = tie.method)
  optimal.value = output.value %>% apply(1, whichMax, tie.method = tie.method)
  return(list(optimal.Tx = optimal.Tx, optimal.value = optimal.value))
}

if (0) {
  a <-
    gk(survival::Surv(T, delta) ~ A + age + cyto + prevTime + prevA + response,
       common.Tx.label = "A", stage.label = 1:2, data = train,
       stage.sep = ".", timepoints = timepoints, 
       splitRule = rule,
       value.crit = value.criterion[1], value.s.time = value.criterion[2],
       ntree = ntree.tmp, nodesize = 3L, mindeath = 1L, 
       ERT = ert, randomSplit = rs * ert)
}
